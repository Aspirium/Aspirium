...
--[[
	...
	--------------
			...
			**TO THOSE WHO ARE CONFUSED IF THE GROUP OR ITEM CONFIGURATION OPTION DOES NOT WORK:**
			...
			-- Kick Message Formats --
				Kick messages can be supplied as a table, or a string. For example:
				BanMessage = {
					"Aspirium System",
					"You have been banned from this server by a moderator",
					"Moderator Name: %moderator%",
					"Expire Time: %expire_time%"
				}

				^ is valid,
				BanMessage = "You are banned. Expires: %expire_time%"

				is also valid.

				Kick Message Placeholders:
					%username% - The name of the player that was kicked
					%userid% - The User Id of the player that was kicked
					%reason% - The reason for the punishment
					%moderator% - The name of the moderator that executed the punishment
					%moderator_id% - The User Id of the moderator that executed the punishment
					%expire_time% - The **UTC TIME** upon which the punishment will expire. For permanent bans, this will be "Never"
					%expire_time_local% - The local time of the user upon which the punishment will expire.
	...
	Settings.Miscellaneous = { --// Random general settings used for minor things
		...
		BanMessage = {
			--// The ban message format must follow the kick message format (Lines 181 - 202)
		},
		
		LockMessage = {
			--// The lock (whitelist) message format must follow the kick message format (Lines 181 - 202)
		},
		...
		AdminNotifications = true, --//Determines if messages such as "You're an admin!" or "System Updated!" Messages show upon a player joining.
		SongHint = true, --// Determines if the Song Id that's being played shows at the top of the screen upon being played.
		TopBarShift = false, --// Determines if hints / other UIs appear at the very top of the player's screen, or if they're offset to be under the topbar.
		
		AutoClean = { --// Cleans the workspace of unnecessary items such as hats and tools.
			Enabled = false,
			Delay = 60 --// Delay in seconds in between each clean.
		},
		
		AutoBackup = false, --// (Not Recommended) Determines if a map backup will be made when the server stuff.
		
		CustomChat = false, --// Determines if the custom Aspirium chat UI is enabled.
		CustomPlayerList = false, --// Determines if the custom Aspirium player list UI is enabled.
		
		Console = true, --// Determines if the client console (command bar) is enabled.
		
		HelpSystem = { --// Allows players to call admins / moderators for help using !help
			Enabled = true,
			SupportRanks = {
				30,
				50,
		...
		InformationButton = { --// Enables a button in the bottom right corner of the screen that shows information about the system.
			Enabled = true,
			Image = "rbxassetid://357249130" --// The image of the button
		},
		
		ToolStorage = ServerStorage --// The default location of tools that can be accessed using ;tools
	}
...
return {Settings = Settings, Descriptions = Descriptions, Order = Order}